    ...
    try {
      if (!isSupabaseConfigured) {
        if (requiresRealWebsiteAudit) {
          toast.error(
            isFr
              ? "Analyse réelle indisponible : la connexion Supabase/Edge Function est requise."
              : "Real analysis unavailable: Supabase/Edge Function connection is required.",
          );
          setResultMode("unavailable");
          setResultMessage(
            isFr
              ? "Aucun rapport n'a été généré, car l'analyse réelle du site n'est pas connectée dans cet environnement. Aucun résultat n'est simulé."
              : "No report was generated because real website analysis is not connected in this environment. No result is simulated.",
          );
          return;
        }
      ...
      if (data?.error) {
        if (requiresRealWebsiteAudit) {
          ...
          setResultMessage(
            data.error ||
              (isFr
                ? "L'analyse réelle du site n'a pas pu être finalisée. Aucun rapport automatique n'a été généré."
                : "The real website analysis could not be completed. No automatic report was generated."),
          );
          return;
        }

        setDiagnosis(buildStrategicDiagnosis("blocked_url_brief", data.error));
        setResultMode("estimate");
        setResultMessage(
          isFr
        ...
        if (requiresRealWebsiteAudit && data.diagnosis.site_accessible !== true) {
          setDiagnosis(null);
          setResultMode("unavailable");
          setResultMessage(
            isFr
              ? "L'analyse réelle du site n'a pas pu confirmer assez de contenu exploitable. Aucun faux diagnostic n'a été généré."
              : "The real website analysis could not confirm enough usable content. No fake diagnosis was generated.",
          );
          return;
        }
        ...
        if (requiresRealWebsiteAudit) {
          ...
          setResultMessage(
            isFr
              ? "L'analyse réelle n'a retourné aucun rapport exploitable. Réessayez ou contactez Pixelrises pour un audit manuel."
              : "The real analysis returned no usable report. Try again or contact Pixelrises for a manual audit.",
          );
          return;
        }
      ...
      if (requiresRealWebsiteAudit) {
        toast.error(
          isFr
            ? "Analyse réelle interrompue. Aucun rapport simulé n'a été généré."
            : "Real analysis interrupted. No simulated report was generated.",
        );
        setDiagnosis(null);
        setResultMode("unavailable");
        setResultMessage(
          isFr
            ? "Le site n'a pas pu être analysé réellement cette fois. Vérifiez l'URL, réessayez, ou demandez un audit manuel Pixelrises."
            : "The website could not be analyzed for real this time. Check the URL, try again, or request a manual Pixelrises audit.",
        );
        return;
      }
    ...
    <section className="landing-section" id="diagnostic">
      ...
      <div className="landing-section-inner">
        ...
        <div className="mx-auto max-w-5xl">
          ...
          {showResult && !isAnalyzing && resultMode === "unavailable" && !diagnosis && (
            <AnimatedSection delay={0.1}>
              <div className="card-premium mx-auto max-w-3xl p-8 sm:p-10">
                <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl border border-orange-400/25 bg-orange-400/10">
                  <AlertTriangle className="h-6 w-6 text-orange-300" />
                </div>
                <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                  {isFr ? "Analyse réelle non finalisée" : "Real analysis not completed"}
                </p>
                <h3 className="text-2xl font-bold">
                  {isFr ? "Aucun faux diagnostic n'a été généré" : "No fake diagnosis was generated"}
                </h3>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">
                  {resultMessage ||
                    (isFr
                      ? "Pixelrises n'affiche pas de rapport si le site n'a pas été analysé réellement."
                      : "Pixelrises does not show a report when the website was not actually analyzed.")}
                </p>

                <div className="mt-6 rounded-2xl border border-primary/15 bg-primary/[0.04] p-5">
                  <p className="text-sm font-semibold text-foreground">
                    {isFr ? "Pour obtenir un vrai diagnostic :" : "To get a real diagnosis:"}
                  </p>
                  <div className="mt-3 space-y-2.5">
                    {[
                      isFr ? "Vérifiez que l'URL est publique et accessible." : "Check that the URL is public and reachable.",
                      isFr ? "Relancez l'analyse après quelques secondes." : "Run the analysis again after a few seconds.",
                      isFr ? "Si le site bloque l'audit, demandez un audit manuel Pixelrises." : "If the website blocks the audit, request a manual Pixelrises audit.",
                    ].map((item) => (
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <Button className="flex-1" onClick={runAnalysis}>
                    {isFr ? "Réessayer l'analyse réelle" : "Try real analysis again"}
                    <Search className="ml-2 h-4 w-4" />
                  </Button>
                  <Button asChild variant="outline" className="flex-1">
                    <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer">
                      <MessageCircle className="mr-2 h-4 w-4" />
                      {isFr ? "Demander un audit manuel" : "Request a manual audit"}
                    </a>
                  </Button>
                </div>
              </div>
