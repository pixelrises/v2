-- One-shot repair for Pixelrises Product Lab admin tables.
-- IMPORTANT: paste and run this whole file in Supabase SQL Editor.
-- Do not run a selected fragment only. Supabase executes highlighted text first.
-- Safe/idempotent: creates or repairs only Product Lab V1/V2 admin review tables.
-- Does not touch credits, payments, Stripe, generated sites, API keys, or frontend secrets.

create extension if not exists pgcrypto;

do $product_lab_roles$
begin
  create type public.app_role as enum ('admin', 'user');
exception
  when duplicate_object then null;
end$product_lab_roles$;
